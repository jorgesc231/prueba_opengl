...
void batch_color_delete_test(void* state)
{
    ; // TODO: Deberia borrar la memoria cuando se vuelve al menu
}

void batch_color_on_update(float dt)
{
    ;
}

void batch_color_render(void* state)
{
    TestBatchColorState* prueba = (TestBatchColorState*)state;
...
void batch_color_imgui_render(void* state)
{
    TestBatchColorState* prueba = (TestBatchColorState*)state;
...
void* create_test_batch_color(Test* current_test)
{
    TestBatchColorState* test_state = (TestBatchColorState*)malloc(sizeof(TestBatchColorState));

    if (test_state)
    {
        test_state->translation = glm::vec3(0.0f, 0.0f, 0.0f);

        // Vertex positions, texture coord, color
        float positions[] = {
           -0.5, -0.5, 0.0f, 0.0f,  0.18f, 0.6f, 0.96f, 1.0f,  // 0
            0.5, -0.5, 1.0f, 0.0f,  0.18f, 0.6f, 0.96f, 1.0f,  // 1
            0.5,  0.5, 1.0f, 1.0f,  0.18f, 0.6f, 0.96f, 1.0f,  // 2
           -0.5,  0.5, 0.0f, 1.0f,  0.18f, 0.6f, 0.96f, 1.0f,  // 3

            0.5, -0.5, 0.0f, 0.0f,  1.0f, 0.93f, 0.24f, 1.0f,  // 4
            1.5, -0.5, 1.0f, 0.0f,  1.0f, 0.93f, 0.24f, 1.0f,  // 5
            1.5,  0.5, 1.0f, 1.0f,  1.0f, 0.93f, 0.24f, 1.0f,  // 6
            0.5,  0.5, 0.0f, 1.0f,  1.0f, 0.93f, 0.24f, 1.0f,  // 7
        };
        ...
        init_vertex_buffer(positions, 8 * 8 * sizeof(float), &test_state->VertexBuffer);
        ...
        init_shader(&test_state->shader, "res/shaders/batch_color.shader");
        ...
        current_test->on_update = batch_color_on_update;
        current_test->on_render = batch_color_render;
        current_test->on_imgui_render = batch_color_imgui_render;
        current_test->delete_test = batch_color_delete_test;
